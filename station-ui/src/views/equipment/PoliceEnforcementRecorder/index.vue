    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['polices:PoliceEnforcementRecorder:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['polices:PoliceEnforcementRecorder:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['polices:PoliceEnforcementRecorder:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-view" @click="handleSelect(scope.row)" 
            v-hasPermi="['polices:PoliceEnforcementRecorder:query']">查看</el-button>
...
export default {
  name: "PoliceEnforcementRecorder",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPoliceEnforcementRecorder(this.queryParams).then(response => {
    ...
    handleSelect(row) {
      ...
      getPoliceEnforcementRecorder(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updatePoliceEnforcementRecorder(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPoliceEnforcementRecorder(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除执法仪记录仪编号为"' + ids + '"的数据项？').then(function() {
        return delPoliceEnforcementRecorder(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('polices/PoliceEnforcementRecorder/export', {
        ...this.queryParams
      }, `PoliceEnforcementRecorder_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
